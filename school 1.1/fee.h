...
public:
    // Constructors
    Fee() : classes(""), year(""), tuitionFee(0.0f), sportsFee(0.0f), uniformFee(0.0f), total(0.0f) {}
    Fee(const string& studentClass, const string& feeYear, float tFee, float sFee, float uFee)
        : classes(student), year(fee), tuitionFee(fee), sportsFee(fee), uniformFee(fee) {
        total = tuitionFee + sportsFee + uniformFee;
    }
    ...
    friend ostream& operator<<(ostream& os, const Fee& fee);
};
